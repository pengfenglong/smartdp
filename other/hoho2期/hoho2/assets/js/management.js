...
var baseUrl = 'https://api.easemob.com/ljs/ljs';
$(function(){
	//设置登录用户
	...
	$(menu[1]).attr('href','_index.html?'+param);
	$(menu[2]).attr('href','yydzgl.html?'+param);
	$(menu[3]).attr('href','yhgl.html?'+param);
	$(menu[4]).attr('href','xqgl.html?'+param);
...
function loadUser(){
	//小区列表
	$.ajax({
		url:baseUrl+'/communities?limit=10000000',
		type:'GET',
		headers: {
			"Authorization" : "Bearer "+getToken()
		},
...
function listCommunties(flag){
	...
	$.ajax({
		url:baseUrl+'/communities?limit=10'+temp,
		type:'GET',
		headers: {
			"Authorization" : "Bearer "+getToken()
		},
...
function loadCommuntie(){
	...
	if(queryString('name')){
		$.ajax({
			url:baseUrl+'/communities/'+decodeURI(queryString('name')),
			type:'get',
			dataType: 'json',
			contentType: "application/json; charset=utf-8",
			headers: {
...
function batcDeleteCommuntie(){
	 $('input[name="selecteds"]:checked').each(function(){  
	   deleteCommuntie($(this).val()); 
	 });  
}

//删除小区
function deleteCommuntie(name){
	$.ajax({
		url:baseUrl+'/communities/'+name,
		type:'DELETE',
		headers: {
			"Authorization" : "Bearer "+getToken()
		},
