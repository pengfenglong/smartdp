	...
	$.ajax({
		...
		success: function(data, textStatus, jqXHR) {
			...
			for(var i=0;i<d.length;i++){
				...
				'<td class="text-center">'+decodeURI(d[i].content)+'</td>'+
	...
	$.ajax({
		...
		success: function(data, textStatus, jqXHR) {
			...
			for(var i=0;i<d.length;i++){
				...
				'<td class="text-center">'+d[i].content+'</td>'+
				'<td class="text-center">'+like_count+'</td>'+
				'<td class="text-center">'+dislike_count+'</td>'+
				'<td class="text-center">'+comment_count+'</td>'+
				'<td class="text-center a-center"><a href="javascript:void(0);">删除</a><a href="yydzxq.html?'+param+'" target="_self">查看</a></td>'+
				'</tr>';
				$('tbody').append(temp);
			}
		}
...
function uploadfile(){
	$('#file').fileupload({
			...
			messages:{
				...
				"acceptFileTypes":"只允许上传.xls文件格式的类型!",
				...
				var url = "http://hohofiles.b0.upaiyun.com/"+data._response.result.url;
					...
					//alert('上传成功！');
					$('#tips').html('上传成功！');
					$('#fileName').html(data._response.result.url.substring(1,data._response.result.url.length));
				}else{
					//alert('上传失败！');
					$('#tips').html('上传失败！');
				}
			...
			$options['save-key'] = "/"+data.files[0].name;
	...
	$.ajax({
		...
		success: function(data, textStatus, jqXHR) {
			...
			for(var i=0;i<d.length;i++){
				...
				var param=getPubParam()+'&id='+d[i].created+'&nickName='+nickName+'&signature='+signature+'&avator='+avator+'&sex='+sex;
				var temp = '<option value="'+nickName+'">'+d[i].username+'</option>';
				$('#users').append(temp);
			}
		}
	});
