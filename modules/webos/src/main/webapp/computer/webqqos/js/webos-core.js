...
//聊天窗口
ChartWin = function(me){
	var userJson={};//{id:name}
	...
	return me = {
		...
		create:function(){
			var chartHTML = '<div>';
			chartHTML += '<div><span>当前用户:[</span>'+loginUserId+']</div>';
			chartHTML += '<div style="margin:5px 0;"><input type="text"/><input type="button" value="搜索"/></div>';
			chartHTML += '<div id="userList" style="height:380px;overflow:auto;margin-bottom:5px;"></div>';
			chartHTML += '<div id="messageList"><span style=y"margin-left:10px;" id="messageBox"></div>';
			chartHTML += '</div>';
			art.dialog({
				id :'chart-window',
				title :'IM',
		...
		refresh:function(){
			$.post('/jwchar/Initialization', function(content) {
				me.handlerContent(content);
				me.refreshUserList();
				me.bindEvent();
			});
		},
		...
		refreshUserList:function(){
			$.post('/jwchar/RefreshUserList', function(content) {
				me.handlerContent(content);
				if(isNeedRefresh){
					me.refreshUserList();
				}
		...
		subMsg:function(userId) {
			var message = $('#message_to_'+userId).val();
			var chartHtml="";
			chartHtml += '<p>';
			chartHtml += userJson[loginUserId];
			chartHtml += ':';
			chartHtml += message;
			chartHtml += '<p>';
			$('#message_content_to_'+userId).append(chartHtml);
			$('#message_to_'+userId).val('');
			$.post('/jwchar/SendMessage', {
				'to' : userId,
				'message' : message
			}, function(content) {
				me.handlerContent(content);
		...
		showMessageBox:function(){
			...
			//发消息对象
			var messageHTML='<div style="overflow:auto;width:100%;height:100%;">';
			var froms =[]; 
		    for (var i=0,l=noReadMessage.length; i<l; ++i){
		    	if($.inArray(noReadMessage[i].from,froms)<0){
		        	froms.push(noReadMessage[i].from);
		    	}
		    }
		    for (var i=0; i<froms.length; i++){
		    	messageHTML += '<p>';
		    	messageHTML += userJson[froms[i]]+':';
		    	for (var j=0; j<noReadMessage.length; j++){
		    		if(noReadMessage[j].from==froms[i]){
		    			messageHTML += '[ ' +noReadMessage[j].content+ ' ]';
		    		}
		    	}
		    	messageHTML += '</p>';
			...
			noReadMessage = [];
			$('#messageBox').html('消息管理器')
		},
		showCharWin:function(userId){
			var html = '<div><div style="overflow-y:auto;width:100%;height:250px;border:1px gray solid;"  id="message_content_to_'+userId+'"></div></div>';
			html += '<div><textarea style="overflow-y:auto" rows="2" cols="28" id="message_to_'+userId+'"></textarea><input type="button" value="发送"/></div>'
			art.dialog({
				id :'chart-win-to-'+userId,
				title :'与'+userJson[userId]+'聊天',
				width :260,
				height:350,
				max:false,
				min:false,
			...
			$('#message_to_'+userId).next().click(function(){
				me.subMsg(+userId);
			});
		},
		handlerContent:function(content) {
			if(content==''){
				return;
			}
			var c = eval('(' + content +')');
			if(c.users){
				...
				for(var u in c.users){
					userJson[c.users[u].id]=c.users[u].name;
					html += '<li>';
					html += '<input name="user" type="radio" id="'+c.users[u].id+'"/>';
					if(c.users[u].state == "0"){
						html += '<span style="color:gray">'+c.users[u].name+'</span>';
					}else{
						html += '<span style="color:red">'+c.users[u].name+'</span>';
					}
					html += '</li>';
				}
				html += '</ul>';
			...
			if(c.messages){
				for(var m=0;m<c.messages.length;m++){
					var chartHtml= '';
					chartHtml += '<p>';
					chartHtml += userJson[c.messages[m].from];
					chartHtml += ':';
					chartHtml += c.messages[m].content;
					chartHtml += '<p>';
					if(chartHtml!=''){
						//没有打开聊天窗口
						if($('#message_content_to_'+c.messages[m].from).length==0){
							var msg = {};
							msg.from = c.messages[m].from;
							msg.content = c.messages[m].content;
							noReadMessage.push(msg);
						}else{
							$('#message_content_to_'+c.messages[m].from).append(chartHtml);
						}
						
					}
				}
				if(noReadMessage.length>0){
					$('#messageBox').html('[有未读消息]')
				}
			}
		}
	}
...
//登录
LoginWin = function(me){
	return me = {
		init:function(){
			me.create();
		},
		create:function(){
			var loginHTML = '<div><div align="right">用户名:<input type="text" id="userId" name="userId"/></div>';
			loginHTML += '<div align="right">密码:<input type="password" id="password" name="password"/></div>';
			loginHTML += '<div align="right"><button onclick="LoginWin.login(this);">登录</button></div></div>';
			art.dialog({
				id :'login-window',
				title :'登录',
				width :320,
		...
		login:function(_this){
			var userId = $(_this).parent().parent().find('#userId').val();
			var password = $(_this).parent().parent().find('#password').val();
			$.ajax({
				url:'/webos/login.action',
				type:'POST',
				...
				success:function(data){
					...
					if(result.code == '0'){
						loginUserId = userId;
						art.dialog.list['login-window'].close();
					}else{
						alert("登录失败");
					}
				}
