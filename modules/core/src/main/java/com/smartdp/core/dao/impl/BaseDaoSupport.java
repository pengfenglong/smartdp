 ...
 */
@Component("baseDao")
public class BaseDaoSupport<T, PK extends Serializable> extends SimpleDao<T, PK> implements IBaseDao<T, PK>{
	/**
	 * 用于Dao层子类的构造函数.
	 ...
	 */
	public BaseDaoSupport(final SessionFactory sessionFactory, final Class<T> entityClass) {
	...
	public Criteria setPageParameterToCriteria(final Criteria c, final DefaultPage<T> page) {
		...
		if (page.isOrderBySetted()) {
			...
			for (int i = 0; i < orderByArray.length; i++) {
				if (DefaultPage.ASC.equals(orderArray[i])) {
