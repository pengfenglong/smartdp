...
public class TreegridTag extends DatagridTag {
	...
	public void populateAttributes() {
		...
		if(StringUtils.isEmpty(id)){
			attributes.put("id", "smart-treegrid");
		}
		attributes.put("treeField", treeField);
	}

	public void populateParams() {
		super.populateParams();
		parameters.put("selectOnCheck", "true");
		...
		parameters.put("url", "'"+getContextPath() + "/" + url + "?" + parentIdField + "='");
		String _s = "function(row,param){if (row){$(this).treegrid('options').url = '"
				+ getContextPath() + "/" + url + "?" + parentIdField + "='+row[$('#smart-treegrid').attr('idField')];}else{$(this).treegrid('options').url='"
				+ getContextPath() + "/" + url + "?" + parentIdField + "=';}}";
		
		parameters.put("onBeforeLoad", _s );
	}
