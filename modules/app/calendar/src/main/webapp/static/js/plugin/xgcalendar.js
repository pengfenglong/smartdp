...
;
(function($) {
	var __WDAY = new Array(i18n.xgcalendar.dateformat.sun,
			i18n.xgcalendar.dateformat.mon, i18n.xgcalendar.dateformat.tue,
			i18n.xgcalendar.dateformat.wed, i18n.xgcalendar.dateformat.thu,
			i18n.xgcalendar.dateformat.fri, i18n.xgcalendar.dateformat.sat);
	var __MonthName = new Array(i18n.xgcalendar.dateformat.jan,
			i18n.xgcalendar.dateformat.feb, i18n.xgcalendar.dateformat.mar,
			i18n.xgcalendar.dateformat.apr, i18n.xgcalendar.dateformat.may,
			i18n.xgcalendar.dateformat.jun, i18n.xgcalendar.dateformat.jul,
			i18n.xgcalendar.dateformat.aug, i18n.xgcalendar.dateformat.sep,
			i18n.xgcalendar.dateformat.oct, i18n.xgcalendar.dateformat.nov,
	...
	if (!dateFormat || typeof (dateFormat) != "function") {
		var dateFormat = function(format) {
			var o = {
				"M+" : this.getMonth() + 1,
				"d+" : this.getDate(),
				"h+" : this.getHours() > 12 ? this.getHours() - 12 : this
						.getHours(),
				"H+" : this.getHours(),
				"m+" : this.getMinutes(),
				"s+" : this.getSeconds(),
				...
				"t" : this.getHours() < 12 ? i18n.xgcalendar.dateformat.AM
						: i18n.xgcalendar.dateformat.PM,
	...
	if (!DateDiff || typeof (DateDiff) != "function") {
		var DateDiff = function(interval, d1, d2) {
			...
			case "w":
				...
				break; // w
			case "h":
				d1 = new Date(d1.getFullYear(), d1.getMonth(), d1.getDate(), d1
						.getHours());
				d2 = new Date(d2.getFullYear(), d2.getMonth(), d2.getDate(), d2
						.getHours());
				break; // h
			case "n":
				d1 = new Date(d1.getFullYear(), d1.getMonth(), d1.getDate(), d1
			...
			case "s":
				d1 = new Date(d1.getFullYear(), d1.getMonth(), d1.getDate(), d1
						.getHours(), d1.getMinutes(), d1.getSeconds());
				d2 = new Date(d2.getFullYear(), d2.getMonth(), d2.getDate(), d2
						.getHours(), d2.getMinutes(), d2.getSeconds());
				break;
			}
			...
			case "d":
				...
				break;
			case "w":
				diff = Math.floor((t2 + 345600000) / (604800000))
	...
	$.fn.bcalendar = function(option) {
		...
		if (eventDiv.length == 0) {
			eventDiv = $("<div id='gridEvent' style='display:none;'></div>")
...
//		}
			...
			if (ll == 0) {
				...
				if (ll > 1) {
					for ( var i = 0; i < ll - 1;) {
						...
						if (diff1 <= 1) {
							d1.startdate = d2.startdate > d1.startdate ? d1.startdate
									: d2.startdate;
							d1.enddate = d2.enddate > d1.enddate ? d2.enddate
									: d1.enddate;
		...
		function render() {
			...
			var showday = new Date(option.showday.getFullYear(), option.showday
					.getMonth(), option.showday.getDate());
		...
		function BuildDaysAndWeekView(startday, l, events, config) {
			var days = [];
			if (l == 1) {
				var show = dateFormat.call(startday,
						i18n.xgcalendar.dateformat.Md);
				days.push({
					display : show,
					date : startday,
				...
				for ( var i = w, j = 0; j < l; i = i + 1, j++) {
					...
					var show = dateFormat.call(ndate,
							i18n.xgcalendar.dateformat.Md);
					days.push({
						display : show,
						date : ndate,
			...
			html
					.push("<table class=\"wk-top\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">");
			...
			html
					.push("<table style=\"height: 1008px\" id=\"tgTable\" class=\"tg-timedevents\" cellspacing=\"0\" cellpadding=\"0\"><tbody>");
		...
		function BuildMonthView(showday, events, config) {
			...
			html
					.push("<table id=\"mvweek\" class=\"mv-daynames-table\" cellSpacing=\"0\" cellPadding=\"0\"><tbody><tr>");
			for ( var i = config.weekstartday, j = 0; j < 7; i++, j++) {
				...
				html.push("<th class=\"mv-dayname\" title=\"", __WDAY[i],
						"\">", __WDAY[i], "");
			}
				...
				for ( var j = 0; j < fE.length; j++) {
					if (!fE[j].crossday && !fE[j].allday) {
						...
						if (da.daystr == fE[j].daystr) {
							...
							if (i == 0 && da.date >= fE[j].event[2]
									&& da.date <= fE[j].event[3])// 跨日的第一个日程
							{
								deB[i].push(fE[j]);
								dMax++;
							}
			...
			for ( var i = 0; i < l; i++) { // 处理全天和跨日的日程
				...
				if (de.length > 0) { // 有日程
					for ( var j = 0; j < de.length; j++) {
						var end = DateDiff("d", lrdate, de[j].event[3]) > 0 ? lrdate
								: de[j].event[3];
		...
		function BuildWT(ht, dayarrs, events, dMax) {
			...
			for ( var i = 0; i < dayarrs.length; i++) {
				...
				if (dateFormat.call(dayarrs[i].date, "yyyyMMdd") == dateFormat
						.call(new Date(), "yyyyMMdd")) {
					ht
							.push(
									"<th abbr='",
									dateFormat
									...
									"' class='gcweekname gcweek-today' scope=\"col\"><div title='",
									title, "' ", ev,
									" class='wk-dayname'><span class='", cl,
									"'>", dayarrs[i].display,
									"</span></div></th>");
				} else {
			...
			// onclick=\"javascript:FunProxy('rowhandler',event,this);\"
			ht
					.push("><div id=\"weekViewAllDaywk\" ><table class=\"st-grid\" cellpadding=\"0\" cellspacing=\"0\"><tbody>");
			...
			if (dMax == 0) {
				...
				for ( var i = 0; i < dayarrs.length; i++) {
					...
					if (sshow == dateFormat.call(new Date(), "yyyy-M-d")) {
						ht.push("<td class=\"st-c st-s st-today\"",
								" ch='qkadd' abbr='", sshow,
								"' axis='00:00'>&nbsp;</td>");
					} else {
						ht.push("<td class=\"st-c st-s\"",
								" ch='qkadd' abbr='", sshow,
								"' axis='00:00'>&nbsp;</td>");
					}
				}
				...
				for ( var j = 0; el < dMax; j++) {
					...
					for ( var h = 0; h < l;) {
						...
						if (e) { // 如果存在
							x[h] = x[h] + 1;
							ht.push("'");
							var t = BuildMonthDayEvent(e, dayarrs[h].date, l
									- h);
							if (e.colSpan > 1) {
				...
				ht.push("<tr>");
				for ( var h = 0; h < l; h++) {
					ht.push("<td class='st-c st-s' ch='qkadd' abbr='",
							dateFormat.call(dayarrs[h].date,
									i18n.xgcalendar.dateformat.fulldayvalue),
							"' axis='00:00'>&nbsp;</td>");
				}
				ht.push("</tr>");
			}
			ht.push("</tbody></table></div></td></tr>"); // stgrid end //wvAd
															// end //td2 end
															// //tr2 end
			// 3:
			ht.push("<tr>");
		...
		function BuildDayScollEventContainer(ht, dayarrs, events) {
			...
			ht
					.push("><div id=\"tgspanningwrapper\" class=\"tg-spanningwrapper\"><div style=\"font-size: 20px\" class=\"tg-hourmarkers\">");
			for ( var i = 0; i < 24; i++) {
			...
			for ( var i = 0; i < 24; i++) {
				...
				ht.push("<div style=\"height: 41px\" class=\"tg-time\">", tmt,
						"</div>");
			}
			ht.push("</td>");
			...
			var l = dayarrs.length;
			for ( var i = 0; i < l; i++) {
				ht.push("<td class=\"tg-col\" ch='qkadd' abbr='", dateFormat
						.call(dayarrs[i].date,
								i18n.xgcalendar.dateformat.fulldayvalue), "'>");
				var istoday = dateFormat.call(dayarrs[i].date, "yyyyMMdd") == dateFormat
						.call(new Date(), "yyyyMMdd");
				// Today
				if (istoday) {
					ht
							.push("<div style=\"margin-bottom: -1008px; height:1008px\" class=\"tg-today\">&nbsp;</div>");
				}
				// var eventC = $(eventWrap);
				// onclick=\"javascript:FunProxy('rowhandler',event,this);\"
				ht
						.push(
								"<div style=\"margin-bottom: -1008px; height: 1008px\" id='tgCol",
		...
		function getTitle(event) {
			...
			timeshow = dateFormat.call(event[2], startformat) + " - "
		...
		function BuildDayEvent(theme, e, index) {
			...
			p.starttime = fomartTimeShow(e.st.hour, e.st.minute,
					option.timeFormat);
			p.endtime = fomartTimeShow(e.et.hour, e.et.minute,
					option.timeFormat);
			p.content = e.event[1];
		...
		function BuilderMonthBody(htb, showday, startday, events, bodyHeight) {
			...
			var firstdate = new Date(showday.getFullYear(), showday.getMonth(),
					1);
			...
			if (enddate.getFullYear() == showday.getFullYear()
			...
			var p = 100 / rc;
			var formatevents = [];
			var hastdata = formartEventsInHashtable(events, startday, 7,
					startdate, enddate);
			var B = [];
			var C = [];
			for ( var j = 0; j < rc; j++) {
				...
				for ( var i = 0; i < 7; i++) {
					...
					C[j * 7 + i] = newkeyDate;
					var newkey = dateFormat.call(newkeyDate,
			...
			for ( var j = 0; j < rc; j++) {
				// onclick=\"javascript:FunProxy('rowhandler',event,this);\"
				htb.push("<div id='mvrow_", j, "' style=\"HEIGHT:", p,
						"%; TOP:", p * j, "%\" class=\"month-row\">");
				htb
						.push("<table class=\"st-bg-table\" cellSpacing=\"0\" cellPadding=\"0\"><tbody><tr>");
				var dMax = B[j];

				for ( var i = 0; i < 7; i++) {
					var day = C[j * 7 + i];
					htb.push("<td abbr='", dateFormat.call(day,
							i18n.xgcalendar.dateformat.fulldayvalue),
							"' ch='qkadd' axis='00:00' title=''");

					if (dateFormat.call(day, "yyyyMMdd") == dateFormat.call(
							new Date(), "yyyyMMdd")) {
				...
				// stgrid
				htb
						.push("<table class=\"st-grid\" cellpadding=\"0\" cellspacing=\"0\"><tbody>");
				...
				for ( var i = 0; i < 7; i++) {
					...
					o.title = dateFormat.call(day,
							i18n.xgcalendar.dateformat.fulldayshow);
					if (day.getDate() == 1) {
						if (day.getMonth == 0) {
							o.dayshow = dateFormat.call(day,
									i18n.xgcalendar.dateformat.fulldayshow);
						} else {
							o.dayshow = dateFormat.call(day,
									i18n.xgcalendar.dateformat.Md3);
						}
					} else {
				...
				htb.push("</tr>");
				var sfirstday = C[j * 7];
				BuildMonthRow(htb, formatevents[j], dMax, roweventcount,
						sfirstday);
		...
		// 格式化月日程格式化
		function formartEventsInHashtable(events, startday, daylength, rbdate,
				redate) {
			var hast = new Object();
			var l = events.length;
			for ( var i = 0; i < l; i++) {
				...
				if (diff > 0) {
					...
					for (; sdtemp <= eD; sD = sdtemp = DateAdd("d", daylength,
							sdtemp)) {
						var d = Clone(s);
						var key = dateFormat.call(sD,
								i18n.xgcalendar.dateformat.fulldaykey);
						...
						// 处理表格跨界，分为跨周和不跨周
						d.colSpan = (x >= daylength) ? daylength
								- DateDiff("d", sdtemp, sD) : DateDiff("d", sD,
								eD) + 1;
						hast[key].push(d);
						d = null;
					}
				} else {
					var key = dateFormat.call(events[i][2],
							i18n.xgcalendar.dateformat.fulldaykey);
					if (hast[key] == null) {
						hast[key] = [];
					}
											...
											tempdata,
											{
												...
												"abbr" : dateFormat
														.call(
																cday[h],
																i18n.xgcalendar.dateformat.fulldayvalue),
												html : i18n.xgcalendar.others
														+ (y[h] - z[h])
														+ i18n.xgcalendar.item,
												click : "javascript:alert('more event');"
											});
							tempCss.push("st-more st-moreul");
							h++;
						} else {
							tempdata.html = BuildMonthDayEvent(e, cday[h], l
									- h);
							tempdata.ch = "show";
							if (e.colSpan > 1) {
		...
		function BuildMonthDayEvent(e, cday, length) {
			...
			var ef = DateDiff("d", cday, e.event[3]) >= length; // e.event[3] >=
																// DateAdd("d",
																// 1, cday);
			if (sf || ef) {
				if (sf) {
			...
			var cen;
			if (!e.allday && !sf) {
				cen = fomartTimeShow(e.st.hour, e.st.minute, option.timeFormat)
						+ " " + e.event[1];
			} else {
				cen = e.event[1];
			}
		...
		function populate() {
			...
			if (option.url && option.url != "") {
				...
				var param = [
						{
							name : "showdate",
							value : dateFormat.call(option.showday,
									i18n.xgcalendar.dateformat.fulldayvalue)
						}, {
							name : "viewtype",
							value : option.view
				...
				$
						.ajax({
							...
							url : option.url,
							data : param,
							dataType : "text", // fixed jquery 1.4 not support
												// Ms Date Json Format
												// /Date(@Tickets)/
							// dataType: "json",
							// dataFilter: function(data, type) { return
							// data.replace(/"\\\/(Date\([0-9-]+\))\\\/"/gi,
							// "new $1"); },
		...
		function fomartTimeShow(h, m, f) {
			var o = {
				...
				"m+" : m,
				"s+" : 0,
				"t" : h < 12 ? i18n.xgcalendar.dateformat.AM
						: i18n.xgcalendar.dateformat.PM
			};
			for ( var k in o) {
		...
		function getymformat(date, comparedate, isshowtime, isshowweek,
				showcompare) {
			var showyear = isshowtime != undefined ? (date.getFullYear() != new Date()
					.getFullYear())
					: true;
			var showmonth = true;
			var showday = true;
		...
		function CalDateShow(startday, endday, isshowtime, isshowweek) {
			if (!endday) {
				return dateFormat.call(startday, getymformat(startday, null,
						isshowtime));
			} else {
				var strstart = dateFormat.call(startday, getymformat(startday,
						null, isshowtime, isshowweek));
				var strend = dateFormat.call(endday, getymformat(endday,
						startday, isshowtime, isshowweek));
				var join = (strend != "" ? " - " : "");
		...
		function checkInEr(start, end) {
			...
			for ( var i = 0; i < ll; i++) {
				...
				if (dateFormat.call(start, "yyyyMMdd") == dateFormat.call(
						dr.startdate, "yyyyMMdd")
						|| dateFormat.call(start, "yyyyMMdd") == dateFormat
								.call(dr.enddate, "yyyyMMdd")) {
					r = true;
				}
				if (!end) {
					r2 = true;
					...
					if (dateFormat.call(end, "yyyyMMdd") == dateFormat.call(
							dr.startdate, "yyyyMMdd")
							|| dateFormat.call(end, "yyyyMMdd") == dateFormat
									.call(dr.enddate, "yyyyMMdd")) {
						r2 = true;
					}
				}
		...
		function buildtempdayevent(sh, sm, eh, em, h, title, w, resize, thindex) {
			var theme = thindex != undefined && thindex >= 0 ? tc(thindex)
					: tc();
		...
		function parseED(data) {
			if (data.length > 6) {
				var e = [];
				e.push(data[0], data[1], new Date(data[2]), new Date(data[3]),
						parseInt(data[4], 10), parseInt(data[5], 10), parseInt(
								data[6], 10), data[7] != undefined ? parseInt(
								data[7], 10) : -1,
						data[8] != undefined ? parseInt(data[8], 10) : 0,
						data[9], data[10]);
				return e;
			}
			return null;
				...
				if (data) {
					if (data.IsSuccess) {
						...
						option.onAfterRequestData
								&& option.onAfterRequestData(3);
					} else {
						option.onRequestDataError
								&& option.onRequestDataError(3, data);
						Ind(de);
						render();
						option.onAfterRequestData
								&& option.onAfterRequestData(3);
			...
			if (data != null) {
				if (option.quickDeleteUrl != "" && data[8] == 1
					...
					if (bud.length == 0) {
						...
						calbutton
								.click(function() {
									var data = $("#bbit-cs-buddle").data(
											"cdata");
									if (option.DeleteCmdhandler
											&& $
													.isFunction(option.DeleteCmdhandler)) {
										option.DeleteCmdhandler.call(this,
												data, quickd);
									} else {
					...
					ss.push(dateFormat.call(data[2],
							i18n.xgcalendar.dateformat.Md3), " (",
							__WDAY[data[2].getDay()], ")");
					if (data[4] != 1) {
						ss.push(",", dateFormat
								.call(data[2], option.timeFormat));
					}

					if (iscos) {
						ss.push(" - ", dateFormat.call(data[3],
								i18n.xgcalendar.dateformat.Md3), " (",
		...
		function moreshow(mv) {
			...
			var day = new Date(arrdays[0], parseInt(arrdays[1] - 1, 10),
					arrdays[2]);
			var cc = $("#cal-month-cc");
		...
		function dayupdate(data, start, end) {
			if (option.quickUpdateUrl != "" && data[8] == 1
					&& option.readonly != true) {
				if (option.isloading) {
					return false;
				}
				...
				var param = [
						...
						},
						{
							...
							value : dateFormat.call(start,
									i18n.xgcalendar.dateformat.fulldayvalue
											+ " HH:mm")
						},
						{
							"name" : "CalendarEndTime",
							value : dateFormat.call(end,
									i18n.xgcalendar.dateformat.fulldayvalue
											+ " HH:mm")
						}, {
							"name" : "timezone",
		...
		function quickadd(start, end, isallday, pos) {
			if ((!option.quickAddHandler && option.quickAddUrl == "")
					|| option.readonly) {
				return;
			}
			...
			if (buddle.length == 0) {
				var temparr = [];
				temparr
						.push('<div id="bbit-cal-buddle" style="z-index: 990; width: 400px;visibility:hidden;" class="bubble">');
				temparr
						.push('<table class="bubble-table" cellSpacing="0" cellPadding="0"><tbody><tr><td class="bubble-cell-side"><div id="tl1" class="bubble-corner"><div class="bubble-sprite bubble-tl"></div></div>');
				temparr
						.push('<td class="bubble-cell-main"><div class="bubble-top"></div><td class="bubble-cell-side"><div id="tr1" class="bubble-corner"><div class="bubble-sprite bubble-tr"></div></div> <tr><td class="bubble-mid" colSpan="3"><div style="overflow: hidden" id="bubbleContent1"><div><div></div><div class="cb-root">');
				temparr
						.push('<table class="cb-table" cellSpacing="0" cellPadding="0"><tbody><tr><th class="cb-key">');
				temparr
						.push(
				...
				temparr
						.push(i18n.xgcalendar.create_event,
				...
				calbutton.click(function(e) {
					...
					if (option.quickAddHandler
							&& $.isFunction(option.quickAddHandler)) {
						...
						$.post(option.quickAddUrl, param, function(data) {
							if (data) {
								if (data.IsSuccess == true) {
									...
									render();
									option.onAfterRequestData
											&& option.onAfterRequestData(2);
								} else {
									option.onRequestDataError
											&& option.onRequestDataError(2,
													data);
									option.isloading = false;
									option.onAfterRequestData
											&& option.onAfterRequestData(2);
						...
						var diff = DateDiff("d", sd, ed);
						newdata.push(sd, ed, allday == "1" ? 1 : 0,
								diff > 0 ? 1 : 0, 0);
				...
				lbtn.click(function(e) {
					if (!option.EditCmdhandler) {
						...
						if (option.EditCmdhandler
								&& $.isFunction(option.EditCmdhandler)) {
							option.EditCmdhandler.call(this, [ '0',
									$("#bbit-cal-what").val(),
									$("#bbit-cal-start").val(),
									$("#bbit-cal-end").val(),
									$("#bbit-cal-allday").val() ]);
		...
		function strtodate(str) {
			...
			var m = arr2[i18n.xgcalendar.dateformat.month_index].indexOf("0") == 0 ? arr2[i18n.xgcalendar.dateformat.month_index]
					.substr(1, 1)
					: arr2[i18n.xgcalendar.dateformat.month_index];
			var d = arr2[i18n.xgcalendar.dateformat.day_index].indexOf("0") == 0 ? arr2[i18n.xgcalendar.dateformat.day_index]
					.substr(1, 1)
		...
		function Ind(event, i) {
			...
			if (!i) {
				if (option.eventItems && option.eventItems.length > 0) {
					...
					var d2 = option.eventItems[sl - 1][2].getTime()
							- s.getTime();
					var diff = d1 - d2;
					if (d1 < 0 || diff < 0) {
						for ( var j = 0; j < sl; j++) {
		...
		function initevents(viewtype) {
			if (viewtype == "week" || viewtype == "day") {
				$("div.chip", gridcontainer).each(
						function(i) {
							...
							if (chip.hasClass("drag")) {
								...
								// resize
								chip.find("div.resizer").mousedown(
										function(e) {
											dragStart.call($(this).parent()
													.parent(), "dw4", e);
											return false;
										});
							} else {
				...
				$("td.st-more", gridcontainer).each(
						function(i) {

							$(this)
									.click(
											function(e) {
												moreshow.call(this, $(this)
														.parent().parent()
														.parent().parent()[0]);
												return false;
											}).mousedown(function() {
			...
			if (_dragdata) {
				...
				case 1:
					...
					if (diffy > 11 || diffy < -11 || d.cpwrap) {
						...
						var tempdata;
						if (!d.cpwrap) {
							tempdata = buildtempdayevent(gh.sh, gh.sm, gh.eh,
									gh.em, gh.h);
							var cpwrap = $(
									"<div class='ca-evpi drag-chip-wrapper' style='top:"
											+ ny + "px'/>").html(tempdata);
							$(d.target).find("div.tg-col-overlaywrapper")
									.append(cpwrap);
							d.cpwrap = cpwrap;
						} else {
							if (d.cgh.sh != gh.sh || d.cgh.eh != gh.eh
									|| d.cgh.sm != gh.sm || d.cgh.em != gh.em) {
								tempdata = buildtempdayevent(gh.sh, gh.sm,
										gh.eh, gh.em, gh.h);
					...
					if (data != null && data[8] == 1) {
						...
						if (diffx > 5 || diffx < -5 || diffy > 5 || diffy < -5
							...
							if (!d.cpwrap) {
								...
								d.top = ny;
								tempdata = buildtempdayevent(gh.sh, gh.sm,
										gh.eh, gh.em, gh.h, data[1], false,
										false, data[7]);
								var cpwrap = $(
										"<div class='ca-evpi drag-chip-wrapper' style='top:"
												+ ny + "px'/>").html(tempdata);
								var evid = d.target.parent().attr("id")
					...
					if (data != null && data[8] == 1) {
						...
						if (diffy != 0 || d.cpwrap) {
							...
							if (!d.cpwrap) {
								...
								if (d.nh != nh) {
									var sp = gP(data[2].getHours(), data[2]
											.getMinutes());
									var ep = sp + nh;
									gh = gW(d.top, d.top + nh);
									tempdata = buildtempdayevent(gh.sh, gh.sm,
											gh.eh, gh.em, gh.h, data[1],
											"100%", true, data[7]);
			...
			if (_dragdata) {
				...
				case 1: // 选择单日的时间段来添加日程
					...
					if (!d.cpwrap) {
						...
						var ny = gP(gh.sh, gh.sm);
						var tempdata = buildtempdayevent(gh.sh, gh.sm, gh.eh,
								gh.em, gh.h);
						d.cpwrap = $(
								"<div class='ca-evpi drag-chip-wrapper' style='top:"
										+ ny + "px'/>").html(tempdata);
						$(d.target).find("div.tg-col-overlaywrapper").append(
								d.cpwrap);
						d.cgh = gh;
					}
					...
					var start = strtodate(d.target.attr("abbr") + " "
							+ d.cgh.sh + ":" + d.cgh.sm);
					var end = strtodate(d.target.attr("abbr") + " " + d.cgh.eh
							+ ":" + d.cgh.em);
					_dragevent = function() {
						$("#" + wrapid).remove();
						$("#bbit-cal-buddle").css("visibility", "hidden");
				...
				case 4: // 单日日程的移动
					if (d.cpwrap) {
						...
						if (start.getTime() == d.data[2].getTime()
		...
		function addlasso(lasso, sdi, edi, xa, ya, height) {
			...
			var i = sp.x;
			var j = sp.y;
			var max = Math.min(document.documentElement.clientWidth,
					xa[l - 1].e) - 2;
